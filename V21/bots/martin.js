        ...
        try {
            ...
            if (allStats[botName]) {
                ...
                botNameContainer.innerHTML = `
                    <img src="${stats.country}" alt="Drapeau de ${botName}" class="flag">
                    <span>${botName}</span>
                `;
...
function playBot() {
    // Ouvrir le bot correspondant dans une nouvelle fenêtre
    const botName = new URLSearchParams(window.location.search).get('bot') || "Martin";
