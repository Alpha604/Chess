...
function openFile(file, filePath) {
  ...
  let contentHTML = '';
  if (file.type === "txt") {
    contentHTML = `<pre class="txt-content" style="padding:10px;color:#e6eef8;">${file.content || ""}</pre>`;
  } else { // iframe ou autre URL
    contentHTML = `<iframe id="${file.name}" src="${file.url}" allowtransparency="true" frameborder="0" style="width:100%; height:calc(100% - 50px);"></iframe>`;
  }
