...
<style>
  ...
  /* bloc version à droite */
  #site-version {
    position: absolute;
    right: 50px;   /* espace depuis la droite */
  }
</style>
`;
...
function injectFooter() {
    ...
    // Récupère le chemin complet (local ou hébergé)
    let path = window.location.pathname;

    // Extrait le dossier parent (ex: "V5")
    let versionFolder = path.split("/").filter(Boolean).slice(-2, -1)[0];

    // Si on a bien trouvé une version
    if (versionFolder) {
